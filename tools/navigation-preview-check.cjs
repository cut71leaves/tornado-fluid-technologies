...
(async()=>{
 ...
 try{
  ...
  for(const width of [1440,390,360]){
   ...
   await page.waitForFunction(()=>document.querySelector('.water-background').dataset.waterState==='active');
   assert.ok(await page.locator('.water-canvas').isVisible());
  ...
  checks.push('Reduced-motion replaces global water with static poster');
